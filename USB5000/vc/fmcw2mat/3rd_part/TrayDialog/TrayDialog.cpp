...
{
	SetForegroundWindow();
	m_mnuTrayMenu.GetSubMenu(0)->TrackPopupMenu(TPM_LEFTALIGN |TPM_RIGHTBUTTON, pt.x, pt.y, this);
	PostMessage(WM_NULL);
...
void CTrayDialog::SwitchFullScreen(void)  
{  
...
void CTrayDialog::SetWinTransparency(int level)
{
...
int CTrayDialog::getMajorVersion()
{
	OSVERSIONINFO osvi;
...
BOOL CTrayDialog::PreTranslateMessage(MSG* pMsg) 
{
    if (WM_LBUTTONDOWN==pMsg->message && pMsg->hwnd == GetSafeHwnd()) 
    {
		PostMessage(WM_NCLBUTTONDOWN, HTCAPTION, MAKELPARAM(GET_X_LPARAM(pMsg->lParam), GET_Y_LPARAM(pMsg->lParam)));
	}
