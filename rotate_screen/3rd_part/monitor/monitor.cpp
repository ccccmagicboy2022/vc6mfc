...
{
	for (vector<CMonitor*>::iterator iter = m_vec_monitor.begin(); iter != m_vec_monitor.end();)
	{
...
{
   for (vector<CMonitor*>::iterator iter = m_vec_monitor.begin(); iter != m_vec_monitor.end(); ++iter)
   {
...
bool CMonitorManager::FullScreenWindow(HWND hWnd, bool bFullScreen, int iMonitorIndex /*= 0*/)
{
	...
	stack<HWND>  WndStack;
    HWND         hParent;
	map<HWND, WndInfo>::iterator iter; 
